...
- ⚠️ **注意系统云备份**：清单中 `android:allowBackup="true"`，Android 的自动备份可能把应用数据库和照片副本纳入云备份（由系统设置决定）。**这条与"自建服务器"无关**：备份走的是用户自己的手机账号（Google Drive 或华为 / 小米等厂商云），由操作系统完成，本项目没有服务器也不需要服务器，开发者无法访问这些备份。应用本身不会主动上传，但"有系统云备份"不等于"数据物理上只存在于本机"。
