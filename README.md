...
- **应用内检查更新**：镜像清单与 GitHub Release 并发查询、取版本更高者（镜像只是国内更快的下载源），下载后交给系统安装器。
...
- 唯一的对外网络请求是更新检查：镜像 `latest.json`（若配置）与 GitHub Release 并发请求、取版本号更高者；只取版本号和下载地址，不发送任何材料内容。
- **系统云备份已关闭**：清单中 `android:allowBackup="false"`，Android 的自动备份不会把应用数据库和照片副本纳入云备份（不会进入 Google Drive 或华为 / 小米等厂商云）。本项目没有服务器也不需要服务器，数据物理上只存在于本机，除非用户主动分享导出的 ZIP。
...
当前版本以 [VERSION_HISTORY.md](VERSION_HISTORY.md) 为准（应用 `versionName` 见 `app/build.gradle.kts`），其中也记录了各版本的开发会话。
